...
public final class CompactPersistentOrderedQueue<T> implements PersistentOrderedQueue<T> {
    ...
    private CompactPersistentOrderedQueue(
    ...
    public static <T extends Comparable<? super T>> CompactPersistentOrderedQueue<T> empty() {
        return new CompactPersistentOrderedQueue<>(naturalOrderComparator(), null, 0, null, null);
    }

    public static <T> CompactPersistentOrderedQueue<T> empty(Comparator<? super T> comparator) {
        return new CompactPersistentOrderedQueue<>(Objects.requireNonNull(comparator, "comparator"), null, 0, null, null);
    }

    @Override
    public CompactPersistentOrderedQueue<T> add(T value) {
        ...
        T updatedLast = size == 0 || compare(requiredValue, require(lastElement), comparator) >= 0
                ? requiredValue
                : require(lastElement);
        return new CompactPersistentOrderedQueue<>(comparator, updatedRoot, sizeOf(updatedRoot), updatedFirst, updatedLast);
    }
    ...
    @Override
    public CompactPersistentOrderedQueue<T> remove(T value) {
        T requiredValue = Objects.requireNonNull(value, "value");
        ...
        if (updatedRoot == null) {
            return new CompactPersistentOrderedQueue<>(comparator, null, 0, null, null);
        }
    ...
    @Override
    public CompactPersistentOrderedQueue<T> replace(T remove, T add) {
