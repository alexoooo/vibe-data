...
final class MemoryUsageReportFormatter {
    ...
    static String toMarkdown(
        ...
        builder.append("# Memory usage summary\n\n")
                ...
                .append("## Methodology\n\n")
                .append("- Retained footprint is measured from the fully built collection instance with JOL `GraphLayout`.\n")
                .append("- Sorted maps are measured in both `ascending` and `descending` order configurations.\n")
                .append("- `Retained bytes` is the full reachable graph size; `structural bytes` subtracts the per-element payload objects while keeping collection nodes, arrays, and references.\n\n")
